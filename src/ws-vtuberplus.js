...
tiktokLiveConnection.getAvailableGifts().then(giftList => {
    let output = ''; // Initialize an empty string to collect the output
    
    giftList.forEach(gift => {
        output += `{id: ${gift.id}, name: "${gift.name}", cost: ${gift.diamond_count}, imageurl: "${gift.image.url_list[0]}"}\n`;
    });

    // Write to a file named 'gifts.txt'
    fs.writeFileSync('gifts.txt', output, 'utf8');

}).catch(err => {
    // In case of an error, log it to the console or save it in another txt file
    console.error(err);
});
